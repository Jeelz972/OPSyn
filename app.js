...
// Parse le type "3pt_arret" en { distance: '3pt', shotType: 'arret' }
...
// Trouve l'angle pour un zoneId
function getZoneAngle(zoneId) {
    const zone = ZONES_CONFIG.find(z => z.id === zoneId);
    return zone ? zone.angle : 90;
}

// Trouve la clé courte (0G, 45D, etc.) pour un zoneId
...
function App() {
    ...
    useEffect(() => {
        ...
        if (h) { 
            ...
            console.log('📂 LocalStorage chargé:', parsed.length, 'entrées');
    ...
    const updateHistory = (newData) => { 
        console.log('💾 Sauvegarde:', newData.length, 'entrées');
        ...
        try {
            ...
            if(mode === 'save') {
                ...
                if(doc.exists) {
                    ...
                    console.log('📥 Firebase chargé:', loadedHistory.length, 'entrées');
                    console.log('   Exemple:', loadedHistory[0]);
                    updateHistory(loadedHistory);
                    updatePlayers(loadedPlayers);
                    alert("✅ " + loadedHistory.length + " entrées chargées !");
        ...
        <div className="min-h-screen pb-12 bg-gray-50 text-gray-800 font-sans">
            <div className="bg-white sticky top-0 z-50 border-b border-gray-200 px-4 py-3 flex justify-between items-center shadow-sm">
                <h1 className="text-xl font-black text-slate-800 flex items-center gap-2">
                    🏀 StatElite <span className="text-xs text-white bg-slate-800 px-2 rounded-full font-normal">v9.1</span>
                </h1>
...
function AnalysisModule({ players, historyData, setHistoryData }) {
    ...
    const stats = useMemo(() => {
        ...
        filteredData.forEach(record => {
            ...
            // Joueur
            if (playersStats[pid]) {
                playersStats[pid].total.tt += tt;
                playersStats[pid].total.tr += tr;
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="text-sm text-gray-500 font-bold">{filteredData.length} séries</div>
            </div>
...
function ShotChartModule({ players, historyData }) {
    ...
    const getPlayerCount = (pid) => {
        return (historyData || []).filter(r => r.playerId === pid).reduce((acc, r) => acc + (r.tentes || 0), 0);
    };
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center">
                            ...
                            <div className="bg-green-50 rounded-lg p-2">
                                ...
                                <div className="text-[10px] text-green-600">Réussis</div>
                            </div>
                            <div className="bg-red-50 rounded-lg p-2">
                                <div className="text-lg font-black text-red-500">{localStats.missed}</div>
                                <div className="text-[10px] text-red-500">Ratés</div>
                            </div>
                        </div>
                    </div>
